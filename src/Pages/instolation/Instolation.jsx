    ...
    return (
        <div className='max-w-[1400px] mx-auto mt-10' >
            ...
            <div className='flex flex-col md:flex-row items-center justify-between mb-6' >
                <p className='text-[#001931] font-semibold md:text-2xl text-lg' >Total Installed app ({installedCard.length})</p>
                <div className="dropdown">
                    <div tabIndex={0} role="button" className="btn m-1">Sort By Size<ChevronDown /></div>
                    <ul tabIndex={0} className="dropdown-content menu bg-base-100 rounded-box z-1 w-52 p-2 shadow-sm">
                        <li className='font-bold' ><a>  descending download  </a></li>
                        <li className='font-bold' ><a>   ascending  download </a></li>
                    </ul>
                </div>
            </div>
            {
                installedCard.map(install=><SinglecCardinstall  handleRemove={handleRemove}  key={install.id}  install={install} ></SinglecCardinstall>)
            }
